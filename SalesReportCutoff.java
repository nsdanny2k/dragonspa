 ...
 * @author nsdan2k
 */
public class SalesReportCutoff extends HttpServlet {
    ...
    */
    ...
    throws ServletException, IOException {
        ...
        transDateFrom = ctr.convertToTimestamp(ctr.convertDateToString(transDate)+" "+selectTimeCutOff);

        Timestamp transDateTo;
        Date dateNext;
        dateNext=ctr.DateNext(ctr.convertDateToString(transDate));
        transDateTo = ctr.convertToTimestamp(ctr.convertDateToString(dateNext)+" "+selectTimeCutOff);
        
        String selectDateFrom=ctr.convertTimeStampToString(transDateFrom);
        String selectDateTo=ctr.convertTimeStampToString(transDateTo);
            ...
            */
            ...
            try {
                ...
                String sql="SELECT SUM( treatment_entry.CHARGE ) AS TOTCASH, SUM("+COMMRATE+") AS TOTCOMM, SUM(IF((NULLIF(treatment.U1,'') IS NULL),0.0,treatment_entry.CHARGE)) AS TOTCARD, treatment.DATE_VISIT, COUNT(treatment.ACCOUNT_ID) AS NUMTRANS "
                        ...
                        +" AND treatment.TREATMENT_ID=treatment_entry.TREATMENT_ID"
                        +" GROUP BY treatment.ACCOUNT_ID"
                        +" ORDER BY treatment.ACCOUNT_ID";
                Statement s=c.createStatement(java.sql.ResultSet.TYPE_SCROLL_SENSITIVE,java.sql.ResultSet.CONCUR_UPDATABLE);
                ResultSet r=s.executeQuery(sql);
                SQLHelper h=new SQLHelper();        
                while (r.next()) {
