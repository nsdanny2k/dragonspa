 ...
 * ProcedureCodes.java
 * 
 * Created on Sep 11, 2007, 12:00:35 PM
 * 
 ...
 * @author nsdan2k
 */
public class ProcedureCodes extends HttpServlet {
    ...
    */
    ...
    throws ServletException, IOException {
        ...
        login.setLastForm("procedurecodes");
        ...
        if (login.getAccountId().equals("system")) {
            page=view.initialize("Item Codes","global.css","System");    
        } else {
            page=view.initialize("Item Codes","global.css");
        }        
// ********************************
// ** TITLE **            
// ********************************            
            page.text("Item Codes", "titletext", "patients.jpg", "", "", "210px", "125px", "300px", "35px", "3", "35px", "35px");
// ********************************
// ** MAIN SUB MENU **            
// ********************************            
            page.startTable("stdgrid","200px","175px","468px","30px","4");
                page.startRow();
                    page.startCell("extralinkcell","1","1","");
                        ...
                        //page.space();
                        //page.space();
                        page.text("New Code", "extralinktext", "insert.jpg", "", "procedurecode?command=2", "35px", "35px");
                    page.endCell();
                page.endRow();
            page.endTable(true);
            ...
            page.startTable("boxgrid","200px","220px","468px","3");
                ...
                model.ProcedureCode code=null;
                model.ProcedureCodes codes=new model.ProcedureCodes();
                int count=codes.fetch(login.getAccountId(),iPg, size);
                for (int i=0; i<codes.size(); i++) {
                    code=(model.ProcedureCode)codes.get(i);
                    page.startRow();
                        page.startCell("", "1", "1", "");
                            page.text(code.getCode(), "extralinktext", "", "", "procedurecode?id="+code.getId()+"&pg="+pg, "", "");
                        page.endCell();
                        page.startCell("", "1", "1", "");
                            page.text(code.getDescription(), "");
                        page.endCell();
                        page.startCell("", "1", "1", "");
                            page.text(""+code.getRate(), "");
                        page.endCell();
                    page.endRow();                
                }                
                        ...
                        if (codes.getSets()>1) {
                            for (int j=1; j<=codes.getSets(); j++) {
                                if (iPg==j) {
                                    ...
                                    page.text("<a href=\"procedurecodes?pg="+j+"\">"+j+"</a>&nbsp;","extralinktext","","","","","");
                                }
                                if ((j % bar)==0) {
                                    page.println("<br/>");
                                }
