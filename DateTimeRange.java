 ...
 * @author nsdan2k
 */
public class DateTimeRange extends HttpServlet {
    ...
    */
    ...
    throws ServletException, IOException {
        ...
        if (selectDate==null) {
            ...
            transDateFrom=ctr.convertToTimestamp(ctr.convertDateToString(ctr.TODAY())+" "+CutOffTime);
            //transDate=ctr.TODAY();
        } else {
            transDateFrom=ctr.convertToTimestamp(selectDate);
            //transDate=ctr.convertToDate(selectDate);
        }
        Timestamp transDateTo;
        if (selectDate==null) {
            //transDateTo=ctr.TIMESTAMP();
            transDateTo=ctr.convertToTimestamp(ctr.convertDateToString(ctr.TODAY())+" "+CutOffTime);
            //transDate=ctr.TODAY();
        } else {
            transDateTo=ctr.convertToTimestamp(selectDate);
            //transDate=ctr.convertToDate(selectDate);
        }
           ...
           page.startTable("stdgrid","200px","175px","468px","30px","4");
                page.startRow();
                    page.startCell("","1","1","");
                        ...
                        page.inputText("selectDateFrom", ctr.convertTimeStampToString(transDateFrom), "151px");
                    page.endCell();
                    page.startCell("","1","1","");
                        page.text("Date To:", "attribtext");
                        page.inputText("selectDateTo", ctr.convertTimeStampToString(transDateTo), "151px");
                    page.endCell();
                    page.startCell("","1","1","");
